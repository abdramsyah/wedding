jQuery(document).ready(function ($) {
  ...
  $(
    '.saic-container-form [name="comment_parent"], .saic-container-form [name="comment_post_ID"]'
  ...
  if (
    typeof jQuery.fn.textareaCount == "function" &&
    SAIC.textCounter == "true"
  ) {
  ...
  $("body").on("click", ".saic-reply-link", function (e) {
    ...
    form
      .find(".saic-textarea")
      .val("")
      .attr("placeholder", SAIC_WP.reply + ". ESC (" + SAIC_WP.cancel + ")")
      .focus();
    form.find('input[name="submit"]').addClass("saic-reply-action");
    $("#commentform-" + post_id)
      .find("input.saic-cancel-btn")
      .show();
    ...
    if ($content.length < 2) {
      ...
      if (form_ok === true) {
        ...
        if (form.find('input[name="submit"]').hasClass("saic-edit-action")) {
          ...
          form.find('input[name="submit"]').hasClass("saic-reply-action")
        ) {
...
function getComments_SAIC(
  ...
  if (num_comments > 0) {
    jQuery.ajax({
      ...
      beforeSend: function () {
        status
          .addClass("saic-loading")
          .html('<span class="saico-loading"></span>')
          .show();
      },
...
function insertComment_SAIC(post_id, num_comments) {
  ...
  jQuery.ajax({
    ...
    success: function (data, textStatus) {
      ...
      if (data != "error") {
        status.html(
          '<p class="saic-ajax-success">' + SAIC.thanksComment + "</p>"
        );
    ...
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      status
        .removeClass("saic-loading")
        .html('<p class="saic-ajax-error" >' + SAIC.duplicateComment + "</p>");
    },
    complete: function (jqXHR, textStatus) {
      setTimeout(function () {
...
function insertCommentReply_SAIC(post_id, comment_id, num_comments) {
  ...
  jQuery.ajax({
    ...
    success: function (data, textStatus) {
      ...
      if (data != "error") {
        status.html(
          '<p class="saic-ajax-success">' + SAIC.thanksReplyComment + "</p>"
        );
        if (link_show_comments.find("span").length) {
        ...
        status.html(
          '<p class="saic-ajax-error">Error in processing your form.</p>'
        );
      }
    },
    error: function (XMLHttpRequest, textStatus, errorThrown) {
      status.html(
        '<p class="saic-ajax-error" >' + SAIC.duplicateComment + "</p>"
      );
    },
    complete: function (jqXHR, textStatus) {
      setTimeout(function () {
...
function editComment_SAIC(post_id, comment_id) {
  ...
  jQuery.ajax({
    ...
    success: function (result) {
      ...
      if (data.ok === true) {
        jQuery("#saic-comment-" + comment_id)
          .find(".saic-comment-text")
...
function cancelCommentAction_SAIC(post_id) {
  jQuery("form#commentform-" + post_id)
    .find('[name="comment_parent"]')
    .val("0");
  jQuery("form#commentform-" + post_id)
    .find(".saic-textarea")
    .val("")
    .attr("placeholder", SAIC.textWriteComment);
  jQuery("form#commentform-" + post_id)
    .find('input[name="submit"]')
    .removeClass();
  jQuery("form#commentform-" + post_id)
...
function rezizeBoxComments_SAIC(wrapper) {
  var widthWrapper = SAIC.widthWrap
    ? parseInt(SAIC.widthWrap, 10)
    : wrapper.outerWidth();
  if (widthWrapper <= 480) {
